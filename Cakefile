...
task "build", "compile haml and coffescript source to javascript and html", ->
    ...
    if fs.statSync(srcpath).isDirectory()
      ...
      for file in files when path.extname(file) is ".haml"
        console.log "Writing file: #{outpath}/#{file.split('.')[0]}.html"
        compiled = haml.render(fs.readFileSync("#{srcpath}/#{file}", "utf-8"))
        fs.writeFileSync("#{outpath}/#{file.split('.')[0]}.html", compiled, "utf-8")
    else
